...
- `useSkillSearch`: Lazy per-language skill-text index across all 16 skill locales (en/zh from the eager bundle; the rest stream in on first query), shared by the arena roster and `/skills`
