...
- **`useGrids` store (`/src/stores/grids.ts`)** — the aggregate. It owns the board array, the active-board pointer (`activeId` / `active`), the globals every board shares (hex size, team view, invert), and the cross-board rules: page-wide character and artifact uniqueness (`findPlacement` / `isUsed`, `findArtifactPlacement` / `isArtifactUsed`), `placeOnActive`, `removeFromAnyBoard`, `dedupeCharacters` (post-restore uniqueness repair), and the drop routers. `routeDrop` gates every character drop through `canDropCharacter` (the predicate the drag-hover cue also reads), then roster and same-board drops use the board's own handler while cross-board drops compose remove + place as compensating transactions; `routeArtifactDrop` does the same for artifacts via `resolveArtifactDrop` / `canDropArtifact`. Paragon levels travel with heroes on cross-board moves, swaps, and board exchanges (`swapBoards`). A successful drop makes the target board active. `setGridCount(n, maps?)` (re)builds the boards, clamped to `[1, MAX_GRID_COUNT]`.
...
A multi-board page renders one `GridContainer` per `GridContext` (each provides its own context); `useGrids` arbitrates across them. The Arena's `GridContainer` is bound to whichever board is _active_, and that instance is replaced when boards are rebuilt (navigating Arena ↔ Teams, or switching team modes, runs `setGridCount`), so the container provides the context through a small forwarding proxy, so descendants always read the live board, never a disposed snapshot.
...
- **Mobile/tablet** — HTML5 drag doesn't fire on touch, so interactions are tap-based, split across two gestures:
  ...
  - **Move / remove (tap-lift, tap-drop)** — tapping a placed hero on the grid **lifts** it (`useSelectionState.liftedHexId`; the source tile tints teal and the hero raises with a bobbing shadow, and the sheet collapses so all cells stay reachable). Then: tapping an empty cell moves it there — same board directly (allowed even at full capacity — a move adds no unit), another board through `routeDrop`, the same cross-board compose drags use; tapping the lifted hero again `removes` it; tapping a _different_ placed hero swaps the two. Both the tap-move and the tap-swap pass `canDropCharacter` first (the same gate drags use), so a team change that would duplicate a hero page-wide silently no-ops. Tapping a non-placement tile cancels the lift. The lift and swap gestures live in `GridCharacters` (the character overlay's tap); the move/target/cancel logic lives in `GridManager`'s `hex:click`.
