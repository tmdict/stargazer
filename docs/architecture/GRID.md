...
2. **Unit mirror-swap.** `swapTeamsAllBoards` (in `stores/urlState`) moves every unit to its 180-degree-rotated tile (`Grid.getRotatedHexId`, `(q,r,s) → (-q,-r,-s)`, e.g. tile 1 ↔ tile 45) on the opposite team, so a formation rotates to the other side keeping its shape (and, with the relabel, its colour). It reuses the URL-restore path (serialize → `mirrorGridState` → rebuild) for exact companion repositioning and phantimal ordering; `mirrorGridState` demotes occupied tile states to available so occupancy is re-derived purely from the placed units (no stale "ghost" tiles). The rotation is an involution, so toggling twice restores the original.

The flag is serialized as a display flag (alongside the swapped positions in `c`/`p`/`a`), so a shared or reloaded inverted board is coherent. Applying the flag on restore only relabels; it does not re-trigger the swap.
