...
- **Plan** (`lib/teams/teamImport.ts`): Pure data mapping, a sibling of `sideLoad.ts`: readings plus overrides to per-board rosters, `issues` and the record name (`S7 - GNX > 10 (1,3,4,5 > 2)`: left player, `>` when they won more maps, each side's map numbers). `isBlockingIssue` separates what would put a wrong roster on a board from what merely leaves something out
