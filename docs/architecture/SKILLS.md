...
One layout, `SkillsBrowser.vue`, backs both the `/skills` index (SPA) and every `/<lang>/skill/<slug>` permalink (SSG). Left column `SkillReader` shows the active hero's `SkillSections` (or an empty prompt on `/skills`); right column `SkillsSelection` is the filterable character grid (text search lives in the ⌘K overlay).
