...
On mobile (≤768px) the roster column becomes a **pull-up bottom sheet** over the reader (`useBottomSheet`): the layout is CSS-driven so the SSG markup hydrates without a mismatch, and the composable adds the drag once mounted (touch or mouse). It opens on the empty `/skills` index and stays peeked on a hero page (so it doesn't cover the skill content); when expanded, a tap-scrim behind it collapses it on tap. This sheet shares its styling and scrim with the grid's roster sheet (`HomeView`'s `.tab-section`) — keep the two in sync.
