...
- **Search**: `useSavedTeamSearch` (shared with the Load menu) takes hero pills plus free text. A team survives when it fields every pill hero and, with text present, the text hits its name at any length or, at 2+ characters, a hero on the boards (`matchCharacterNames`, the roster's multi-locale index); pill and text-matched heroes get a ring in the thumbnail. Phantimals and companion summons never match. The box shows only with 2+ teams, and hiding it clears pills and text so the list cannot strand on "no matches"
- **Search field** (`SavedTeamsSearch`; the Load menu keeps a text-only input): text of 2+ characters that matches a hero name lists, under the box, the matching heroes fielded by the teams the pills leave (at most 8, prefix hits first), so a pick can never empty the list. Enter or a click turns the lit hero into a pill and clears the text for the next term; team names never become pills. Backspace on empty text drops the last pill, Escape closes the list, and the X clears pills and text together, shown whenever there is anything to clear
