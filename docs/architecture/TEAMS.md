...
- **Per-board actions**: swap (drag to reorder, via `useGridSwap`), copy image, download image, clear (two-step armed confirm, like every destructive control)
...
`TeamImportButton` (`/src/components/teams/TeamImportButton.vue`), an icon-only control after the Load menu, opens `TeamImportModal` (loaded on first use). The modal reads the game's match result screenshots on the device, shows every reading in a review grid, and saves the result as a new library record: both rosters with each hero's paragon and refinement, both artifacts, and the record name in the export convention (`S7 - GNX > 10 (1,3,4,5 > 2)`). The readers, the review state, the plan and the reference images are documented in [IMPORT_TEAM.md](./IMPORT_TEAM.md); the parts that belong to this page:

- **Apply**: `grids.applyRosters(plan)` clears both sides of every mapped board first (so a hero moving between boards never trips page-wide uniqueness against its own old copy), re-seeds each board's phantimal baseline (the `swapBoards` idiom, so the reconcile watcher derives the phantimal even when the old roster had the same one), then auto-places each hero with its attribute record and sets the artifacts, skipping anything page-wide uniqueness already claims. Like the side-load it is an engine-primitive edit, not a restore: maps and display flags are untouched. `rostersWouldReplace` is its read-only mirror for the two-step confirm
- **Save as New**: `TeamsView.handleImportMatch` applies the plan, waits a tick for the phantimals, and saves the boards as a new library record under the record name, which becomes the boards' title with the record as their source (so the next Save updates it, and the record the boards came from is never touched). A full library leaves the boards as an unsaved team that still carries the name as its title (`pendingName`) and Save as New prefill
