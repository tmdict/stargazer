...
- **Reading**: `useTeamImport` (`/src/composables/useTeamImport.ts`, module-level state so the modal can close and reopen) decodes each file to a 1150 px wide RGBA buffer and hands it to `teamImport.worker.ts`, which runs `lib/import` (pure, canvas-free, also driven by `npm run check:import` over a folder of screenshots). The readers anchor the two portrait columns by scoring all five card frames together, match each card's frame (paragon level and exact box), correlate the face crop against a descriptor table built from the 180 px portraits (`loadMatcherPortraits`, fetched on first use), count and colour the star row (refinement; fewer than six stars is R0), locate the map strip's green ring by geometry, and match the header artifact icons. Frames come from chaldea (`importReferenceUrl`), artifact icons from the bundled six plus the seasonal set already on chaldea, and costume references from chaldea's `img/import/skin/` as listed in `img/import/skins.json` (`{ "<hero slug>": ["<file>", …] }`; a missing manifest means none). A costume reference is a capture of the game's skin card, where the face sits smaller and higher than in the bundled art, so it is kept at full size and cut over a second, tighter window grid as well as the art grid; a hero's best window from any of its portraits counts
