...
`TeamImportButton` (`/src/components/teams/TeamImportButton.vue`), an icon-only control after the Load menu, opens `TeamImportModal` (loaded on first use), which reads the game's match result screenshots on the device and saves what they show as a new library record. The readers, the review, the plan and the reference images are documented in [IMPORT_TEAM.md](./IMPORT_TEAM.md); this page owns:

- **Apply**: `grids.applyRosters(plan)` stamps the plan's rosters with engine primitives, not a restore, so `sourceId`, maps and display flags stay untouched; `rostersWouldReplace` mirrors it for the two-step confirm and `rosterConflicts` previews what page-wide uniqueness will skip
- **Save as New**: `TeamsView.handleImportMatch` applies the plan, waits a tick for the phantimals, and saves the boards as a new record under the record name, which becomes their title and source; a full library leaves an unsaved team carrying the name (`pendingName`), cleared by New, Load, Save as New or a mode switch
