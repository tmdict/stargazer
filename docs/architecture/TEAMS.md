...
- **Row 2 (act)**: team actions, the side-load menu and the icon-only screenshot import, then the share actions (link, copy, download, clear)
...
### TeamPowerPanel (`/src/components/grid/TeamPowerPanel.vue`)

The hero panel under each board: one block per side listing its real heroes (portrait, name, upgrade pill), mounted under Grid Info's Hero card toggle in `GridBoard` on Teams and `HomeView` on the Arena, and read-only on `/share` (visibility follows the viewer's own prefs there, as on every page). An empty side is omitted and team view hides the enemy block.

- **Split pill**: with the Upgrades toggle on, each portrait carries one pill, paragon on the left half and refinement on the right, divided by a slanted white seam that stays visible at every level so the pill always reads as two halves. Fills follow `pillTone` (`/src/lib/characters/upgradeStats.ts`, shared with the guide's level headers): gray for P0 to P3 and R0 to R1, a light red tint from R2, and white text on colour at max (silver-blue P4, red R4), via the `--upgrade-pill-*` variables
- **Tap to edit**: a hero entry is one button; tapping it edits the layers the dock's selector has armed (see TeamPowerDock). A single layer steps up by one and wraps from max to 0. ALL raises every layer by one, clamped at max, and wraps them all to 0 together only once every layer is maxed, so two counters at different values can never desync. Taps are disabled on read-only pages and while the Upgrades toggle is off
- **Rivalry stat**: with Upgrades on, each block's header shows the side's net Rivalry-mode stat: Inspiration when positive, Intimidation when negative, with the in-game description as an info tooltip (label and tooltip disappear at 0). Driven by paragon alone (`/src/lib/characters/paragon.ts`, the rivalry ramp in `upgradeStats.ts`): each hero grants its team equal Inspiration and Intimidation, so a side's net is its total minus the enemy's and the two sides always mirror

### TeamPowerDock (`/src/components/grid/TeamPowerDock.vue`)

Each board's upgrade-edit bar, mounted beside the hero panel under the same `info.heroCard && !readonly` gate (in `GridBoard` on Teams, directly in `HomeView` on the Arena; ShareView renders none). Per-team clear and bulk buttons (reset / +1 / max) flank a centered ALL / P / R layer selector, so destructive and bulk actions always sit next to the heroes they change; the enemy cluster hides under team view and an empty side's buttons disable. The layer choice itself is one page-global value (`/src/composables/useAttrLayerSelection.ts`, a module singleton like `useSelectionState`): the chips are mutually exclusive (ALL edits every visible layer), a chip picked on any dock flips every dock and panel, and portrait taps edit the chosen layer restricted to the visible ones (a hidden choice falls back to the visible layers; the single Grid Info "Upgrades" toggle shows both layers or none, so with it off nothing is editable). The lit chip is derived from that effective set, so it always equals what taps will edit.
...
`TeamImportButton` (`/src/components/teams/TeamImportButton.vue`), an icon-only control after the Load menu, opens `TeamImportModal` (loaded on first use). The modal reads the game's match result screenshots on the device and saves what they show as a new team on the mapped boards: both rosters, each hero's paragon and refinement, and both artifacts.

- **Reading**: `useTeamImport` (`/src/composables/useTeamImport.ts`, module-level state so the modal can close and reopen) decodes each file to a 1150 px wide RGBA buffer and hands it to `teamImport.worker.ts`, which runs `lib/import` (pure, canvas-free, also driven by `npm run check:import` over a folder of screenshots). The readers start from the two landmarks every result screen has: the Ally and Enemy tabs, each a solid orange or blue block (that team's result on this map) at a panel's top left beside the panel's dark header row. Each team's cards start just under its tab, so the five-frame comb only settles the exact column position there (sideways more widely, since a taller phone puts the column further from the panel's edge). Without tabs, the columns are tried at their crop positions, then found as a pair anywhere in the shot from a single-frame score profile with pitch and gap free, the solid summary bar under each fifth card telling a real column from one started a row too high. The one invariant is the width: a crop must keep the full screen width, since the game lays the panel out from it. They then match each card's frame at every pixel within a few of its row (paragon level and exact box), correlate the face crop against a descriptor table built from the 180 px portraits (`loadMatcherPortraits`, fetched on first use), count and colour the star row (refinement; fewer than six stars is R0), locate the map strip's green ring (each circle's ring band scored for green over the rows and a little sideways slack below the enemy panel; the ring's position and the outer discs say whether the strip has three or five circles, so a match read in the wrong mode is flagged rather than misnumbered; a one-map mode has no strip and the one board is the map), and match the header artifact icons. The scanned pair is trusted down to a lower floor than the nominal anchor, since two combs a panel gap apart with a summary bar under each are strong structure and a capture taken while the panel was still fading in scores its frames low. A card column whose frames match nothing is reported as such and its cells are offered empty for the picker instead of a stranger's face; a screenshot that ends up with no heroes on either side is skipped by the plan. Frames come from chaldea (`importReferenceUrl`), artifact icons from the bundled six plus the seasonal set already on chaldea, and costume references from chaldea's `img/import/skin/` as listed in `img/import/skins.json` (`{ "<hero slug>": ["<file>", …] }`; a missing manifest means none). A costume reference is a capture of the game's skin card, where the face sits smaller and higher than in the bundled art, so it is kept at full size and cut over a second, tighter window grid as well as the art grid; a hero's best window from any of its portraits counts
- **Confidence**: a hero is sure when its score leads the runner-up by 0.10, or by 0.15 when the winning window came from a costume reference (its looser framing and wider search fit a stranger's face more closely than the art grid can); below that the cell is amber, and a cell under the score floor is red and contributes nothing until picked. A frame match below its floor turns the paragon amber. Every reading shows in the review grid before anything reaches a board; a cell's name opens the on-grid picker's own palette (`CharacterSelectionPalette`, shared with `CharacterSelectionPopup`: name search, faction filter, Enter takes a single match) in a `SelectionPopup` layered over the modal, with the reader's candidates pinned in a first row; each side's artifact chip (icon and name, amber while unsure) opens `ArtifactSelectionPalette`, shared with `ArtifactSelectionPopup`, the same way
- **Learned icons**: a costume with no reference on chaldea defeats the bundled art. Correcting an amber or red cell stores its face descriptor under the picked hero (`stargazer.import.learned`, Int8-quantised, capped, with the descriptor geometry written into the envelope so a change to the crop discards the store); the worker rebuilds its table and matches that icon from then on. A learned match is never sure on first sight unless it leads the best bundled candidate by the usual margin. "Forget learned icons" clears the store
- **Mapping**: each card shows a thumbnail (`ImageLightbox` on click: fitted to the viewport, click for 1:1, layered over the modal) and two labelled controls: Map (the board it fills, read from the strip, sized by `TEAM_MODES[mode].boardCount`) and Winner (read from the Ally tab's colour, the segments carrying the typed player names in the game's orange/blue result colours); both are editable and the winner feeds the record name. Two cards on one map, or the same hero on the same side of two maps (a hard game rule, so it means a misread or a foreign screenshot), block Save as New
- **Plan and save**: `buildTeamImportPlan` (`/src/lib/teams/teamImport.ts`, a sibling of `sideLoad.ts`) turns readings plus overrides into per-board `BoardRoster`s and the record name in the export convention (`S7 SL5 Group - GNX > 10 (1,3,4,5 > 2)`: left player first, `>` when they won more maps, each side's map numbers in the brackets, dropped for a one-map mode; names may contain spaces, never `<>()`). `grids.applyRosters` clears both sides of every mapped board first (so a hero moving between boards never trips page-wide uniqueness against its own old copy), re-seeds each board's phantimal baseline (the `swapBoards` idiom, so the reconcile watcher derives the phantimal even when the old roster had the same one), then auto-places each hero with its attribute record and sets the artifacts, skipping anything page-wide uniqueness already claims. Like the side-load it is an engine-primitive edit, not a restore: maps and display flags are untouched
- **Reference images**: chaldea's `img/import/` holds each reference as a PNG (the source of truth, never fetched) beside the WebP the app fetches: the paragon frames, the star rows, and `skin/<hero slug>-<name>.png` for costume captures. `npm run import:refs -- --chaldea <checkout>` rewrites every WebP from its PNG and regenerates `skins.json` from the skin folder, so adding a costume is: drop the PNG, run it, commit chaldea. `npm run check:import -- --frames <checkout>/img/import …` picks the costumes up from the same folder
- **Save as New**: `TeamsView` applies the plan, waits a tick for the phantimals, and saves the boards as a new library record under the record name, which becomes the boards' title with the record as their source (so the next Save updates it, and the record the boards came from is never touched). A full library leaves the boards as an unsaved team that still carries the name as its title and Save as New prefill. The remembered prefix and player names live in `stargazer.import.names`
