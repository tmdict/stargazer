...
- **After Replace**: `TeamsView` detaches provenance (the title reads "Unsaved team", Save degrades to Save as New) and offers the record name through the Save as New popover until the boards are saved, cleared, or loaded. The remembered prefix and player names live in `stargazer.import.names`
