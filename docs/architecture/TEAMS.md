...
`TeamImportButton` (`/src/components/teams/TeamImportButton.vue`), an icon-only control after the Load menu, opens `TeamImportModal` (loaded on first use). The modal reads the game's match result screenshots on the device and replaces the mapped boards with what they show: both rosters, each hero's paragon and refinement, and both artifacts.
...
- **After Replace**: `TeamsView` detaches provenance (Save degrades to Save as New) and carries the record name as the boards' title, dimmed with an "Unsaved team" status, and as the Save as New prefill until the boards are saved, cleared, or loaded. The remembered prefix and player names live in `stargazer.import.names`
