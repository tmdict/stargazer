...
- **Confidence**: a hero is sure when its score leads the runner-up by 0.10; below that the cell is amber with a picker, and a cell under the score floor is red and contributes nothing until picked. A frame match below its floor turns the paragon amber. Every reading shows in the review grid before anything reaches a board
...
- **Mapping**: each card carries the map the strip named, editable as a segmented control sized by `TEAM_MODES[mode].boardCount`; two cards on one map, or the same hero on the same side of two maps (a hard game rule, so it means a misread or a foreign screenshot), block Replace
